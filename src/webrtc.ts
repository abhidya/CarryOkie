...
interface RelayedStream { sourcePeerId: string; stream: MediaStream; }
...
export class PeerNode extends EventTarget {
  ...
  constructor(localPeerId: string) { super(); this.localPeerId = localPeerId; this.peers = new Map(); this.clockOffsetMs = 0; this.localStreams = []; this.relayedStreams = []; }
  makeConnection(remotePeerId: string, {manual = true, initiator = false, replace = false}: {manual?: boolean; initiator?: boolean; replace?: boolean} = {}): PeerEdge {
    assertWebRtcSupported();
    const existing = this.peers.get(remotePeerId);
    if (existing && !replace) return existing;
    if (existing && replace) { existing.pc.close?.(); this.peers.delete(remotePeerId); }
    ...
    pc.onnegotiationneeded = () => { void this.negotiate(edge); };
  ...
  attachChannel(edge: PeerEdge, dc: RTCDataChannel): void {
    edge.dc = dc;
    dc.onopen = () => { this.emit('open', {remotePeerId:edge.remotePeerId}); if (edge.streams.length) this.requestNegotiation(edge); };
  ...
  handleMessage(remotePeerId: string, msg: Record<string, unknown>): void {
    ...
    if (msg.type === RPC.SIGNAL_RELAY_OFFER && msg.toPeerId === this.localPeerId) { void this.acceptRenegotiationOffer(remotePeerId, msg).catch(e => this.emit('error', {message:e.message, remotePeerId})); return; }
    if (msg.type === RPC.SIGNAL_RELAY_ANSWER && msg.toPeerId === this.localPeerId) { void this.acceptRenegotiationAnswer(remotePeerId, msg).catch(e => this.emit('error', {message:e.message, remotePeerId})); return; }
    this.emit('message', {remotePeerId, msg});
  }
  signalDescription(msg: Record<string, unknown>): RTCSessionDescriptionInit {
    const signal = msg.signal as RTCSessionDescriptionInit | {description?: RTCSessionDescriptionInit};
    return ((signal as {description?: RTCSessionDescriptionInit})?.description || signal) as RTCSessionDescriptionInit;
  }
  async acceptRenegotiationOffer(remotePeerId: string, msg: Record<string, unknown>): Promise<void> {
    const edge = this.peers.get(remotePeerId) || this.makeConnection(remotePeerId, {manual:false, initiator:false});
    ...
    this.send(remotePeerId, {type:RPC.SIGNAL_RELAY_ANSWER, fromPeerId:this.localPeerId, toPeerId:remotePeerId, signal:edge.pc.localDescription});
  }
  async acceptRenegotiationAnswer(remotePeerId: string, msg: Record<string, unknown>): Promise<void> {
  ...
  async negotiate(edge: PeerEdge): Promise<void> {
    ...
    try {
      ...
      await waitForIceComplete(edge.pc);
      this.send(edge.remotePeerId, {type:RPC.SIGNAL_RELAY_OFFER, fromPeerId:this.localPeerId, toPeerId:edge.remotePeerId, signal:edge.pc.localDescription});
    } catch (err) {
      edge.negotiating = false;
      throw err;
    }
  ...
  async createManualOffer(remotePeerId: string) {
    const edge = this.makeConnection(remotePeerId, {manual:true, initiator:true, replace:true});
    const offer = await edge.pc.createOffer({offerToReceiveAudio:true});
  ...
  async acceptManualOffer(text: string) {
    ...
    const edge = this.makeConnection(payload.fromPeerId, {manual:true, initiator:false, replace:true});
  ...
  relayRemoteStream(sourcePeerId: string, stream: MediaStream): void {
    if (!this.relayedStreams.some(s => s.sourcePeerId === sourcePeerId && s.stream === stream)) this.relayedStreams.push({sourcePeerId, stream});
