...
function makeCodewords(text: string): number[] {
  ...
  for (const size of BLOCK_SIZES) {
    const dat = data.slice(off, off + size); off += size;
...
function set(m: (boolean | null)[][], r: number, c: number, v: boolean | null): void { if (r >= 0 && r < SIZE && c >= 0 && c < SIZE) m[r][c] = !!v; }
function reserve(m: (boolean | null)[][], r: number, c: number, v = false): void { set(m, r, c, v); }
...
function addFunctionPatterns(m: (boolean | null)[][]): void {
  addFinder(m, 0, 0); addFinder(m, 0, SIZE - 7); addFinder(m, SIZE - 7, 0);
  for (let i = 8; i < SIZE - 8; i++) { set(m, 6, i, i % 2 === 0); set(m, i, 6, i % 2 === 0); }
  for (const r of ALIGN) for (const c of ALIGN) addAlignment(m, r, c);
  set(m, 4 * VERSION + 9, 8, true);
  for (let i = 0; i < 9; i++) { reserve(m, 8, i); reserve(m, i, 8); }
...
export function qrMatrix(text: string): boolean[][] {
  const m = blankMatrix();
  addFunctionPatterns(m); placeData(m, makeCodewords(text)); addFormat(m);
  return m.map(row => row.map(v => !!v));
}
export function qrSvg(text: string, { scale = 4, quiet = 4, title = 'CarryOkie QR' } = {}): string {
  const m = qrMatrix(text);
  const n = m.length + quiet * 2;
  const rects = [];
  m.forEach((row, r) => row.forEach((v, c) => { if (v) rects.push(`<rect x="${c + quiet}" y="${r + quiet}" width="1" height="1"/>`); }));
