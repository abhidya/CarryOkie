  ...
  return `<ul>${room.queue
    .map((queueItem) => {
      ...
      const phoneControls = !["active", "ended"].includes(queueItem.status)
        ...
        : "";
      return `<li>${escapeHtml(queueItem.status)}: ${escapeHtml(songTitle(queueItem.songId))} singers ${escapeHtml(queueItem.singerNumbers.join(","))} ${mode === "host" ? hostControls : phoneControls}</li>`;
    })
    .join("")}</ul>`;
}
