...
function resolveAppAssetUrl(url: string | null | undefined): string | null {
  ...
  const publicPath = url.startsWith("/public/")
...
function resolveEncryptedMedia(
  media: EncryptedMedia | undefined,
): EncryptedMedia | undefined {
...
export async function loadProtectedCatalog(
  catalogUrl: string | URL = resolveAppAssetUrl("/protected/catalog.json")!,
): Promise<ProtectedSong[]> {
  try {
...
export async function decryptProtectedMedia(
  song: ProtectedSong,
): Promise<string | null> {
