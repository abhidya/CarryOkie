...
import { PeerNode, RPC, assertWebRtcSupported, rtcConfig, waitForIceComplete } from './webrtc.ts';
...
let room = loadRoom(); let player = JSON.parse(localStorage.getItem('carryokie.player') || 'null'); let peerNode; let audio; let catalog=[]; let castController; let castListenersAttached = false; let phoneSyncTimer = null; let receiverChannel; let receiverPc; let receiverSessionId = null; let receiverAudioDirty = false; let receiverNegotiating = false; let receiverPendingRenegotiate = false; const receiverStreams = new Set();
...
function receiverUrl(){ return new URL(`../receiver/?room=${encodeURIComponent(room?.roomCode || '')}`, location.href).toString(); }
function receiverPayload(){ return { roomCode:room?.roomCode, queue:queuePreview(), singers:room?.players?.filter(p=>p.isSingerForCurrentSong) || [], song:currentSong(), playbackState:room?.playbackState }; }
function publishReceiverState(){ receiverChannel?.postMessage?.({type:'RECEIVER_STATE', payload:receiverPayload()}); }
function publishReceiverPlayback(sample=room?.playbackState){ receiverChannel?.postMessage?.({type:'RECEIVER_PLAYBACK_SYNC', payload:sample}); }
function publishReceiverCommand(type, payload={}){ receiverChannel?.postMessage?.({type, payload}); }
function sendCastRoomUpdate(type, payload={}){ castController?.sendSafe?.(type, payload); publishReceiverState(); }
function resetReceiverAudio(receiverId){ receiverPc?.close?.(); receiverPc=null; receiverStreams.clear(); receiverSessionId=receiverId; receiverAudioDirty=true; receiverPendingRenegotiate=false; }
async function negotiateReceiverAudio(){
 if(!player?.isHost || !receiverPc || !peerNode || !receiverAudioDirty) return;
 if(receiverNegotiating || receiverPc.signalingState !== 'stable'){ receiverPendingRenegotiate = true; return; }
 ...
 try{
  ...
  receiverChannel?.postMessage({type:'RECEIVER_OFFER', receiverId:receiverSessionId, description:receiverPc.localDescription});
  offerSent = true;
 } finally { if(!offerSent) receiverNegotiating = false; }
}
 ...
 receiverChannel.onmessage = async ev => {
  ...
  if(msg.type === 'RECEIVER_READY'){
   ...
   if(!receiverPc){
    ...
    receiverPc.oniceconnectionstatechange = () => log(`Receiver tab audio ${receiverPc.iceConnectionState}`);
   }
   negotiateReceiverAudio().catch(e=>log(e.message));
  }
  if(msg.type === 'RECEIVER_ANSWER' && receiverPc && (!msg.receiverId || msg.receiverId===receiverSessionId)){ await receiverPc.setRemoteDescription(msg.description).catch(e=>log(e.message)); receiverNegotiating = false; if(receiverPendingRenegotiate || receiverAudioDirty) negotiateReceiverAudio().catch(e=>log(e.message)); }
 };
}
...
function attachCastListeners(cast){
 ...
 cast.addEventListener('playbackSample',e=>{ room.playbackState={...room.playbackState,...e.detail,syncDegraded:false,lastUpdatedAtHostMs:Date.now()}; peerNode?.broadcast({type:RPC.PLAYBACK_SYNC, sample:room.playbackState}); publishReceiverPlayback(room.playbackState); persist(); });
}
...
export async function hostPage(root){ await loadCatalog(); if(!player?.isHost){ player=makePlayer('host','Host'); player.playerNumber=1; room=makeRoom(player); persist(); } setupPeer(player.peerId); setupReceiverBridge(); commonChrome(root,'Host Controller'); renderHost($('#main')); }
function renderHost(main){
 ...
 main.innerHTML = `<section class="grid"><details class="card"${setupOpen}><summary>${roomSummary}</summary><p>Share this with singers after opening the room.</p><p><a href="../player/?room=${escapeHtml(room.roomCode)}">Player join link</a></p><p><a href="${escapeHtml(receiverUrl())}" target="_blank" rel="noreferrer">Open TV receiver tab</a></p><p class="hint">For Chrome tab casting: open this receiver tab, then Cast tab. It will show this room, queue, singers, and live mic audio from the host tab.</p><button id="newRoom">New room</button>${tvBleedWarn}</details><details class="card"${setupOpen}><summary>Pair phones</summary><p>Only use this when adding another phone. Player creates an offer, host returns the answer.</p><textarea id="offer" placeholder="Paste player offer/link/chunks"></textarea><div class="button-row"><button id="scanOfferQr">Scan player QR</button><button id="answerOffer" class="primary">Create host answer</button></div><div id="answerOut"></div></details><details class="card"${castOpen}><summary>TV cast controls</summary><p id="castStatus">Click to connect to Chromecast</p><label>Cast media origin <input id="castOrigin" value="${escapeHtml(castOrigin())}" placeholder="http://192.168.x.x:4174"></label><p class="hint">Default Chromecast receiver plays media only. To show room/queue and hear live mics, open the TV receiver tab link above and Cast tab from Chrome.</p><button id="castBtn">Cast current song to TV</button><button id="castLoadBtn" style="display:none">Reload current song on TV</button><button id="castPlayBtn" style="display:none">Play</button><button id="castPause" style="display:none">Pause</button><label>Seek seconds <input id="castSeekSeconds" type="number" min="0" value="0"></label><button id="castSeek" style="display:none">Seek</button><pre id="castState"></pre></details><div class="card"><h2>Queue</h2>${queueHtml(room,'host')}<div class="button-row"><button id="acceptAll">Accept all</button><button id="startNext" class="primary">Start next queued</button></div></div><details class="card"${setupComplete ? '' : ' open'}><summary>Singers / mic control</summary>${room.players.map(p=>`<label><input type="checkbox" class="singer" value="${escapeHtml(p.playerId)}" ${p.isSingerForCurrentSong?'checked':''}> #${p.playerNumber} ${escapeHtml(p.displayName)}</label><button class="mutePlayer" data-player-id="${escapeHtml(p.playerId)}">Mute #${p.playerNumber}</button>`).join('')}<button id="setSingers" class="primary">Set singers</button></details></section>`;
  ...
  $('#castBtn').onclick=async()=>{ try{ saveCastOrigin(); $('#castBtn').disabled=true; $('#castStatus').textContent='Connecting to Chromecast…'; await cast.init(); await cast.requestSession(); $('#castBtn').style.display='none'; showCastControls(); $('#castStatus').textContent='Connected to TV'; publishReceiverState(); log('Cast connected'); await loadCurrentSongOnTv(); }catch(e){ $('#castBtn').disabled=false; log(e.message); } };
  ...
  $('#castSeek').onclick=()=>{ const seconds=+$('#castSeekSeconds').value || 0; publishReceiverCommand('CAST_SEEK',{seconds}); cast.seek(seconds); };
}
export async function playerPage(root){ await loadCatalog(); if(!player?.playerId || player.isHost){ player=makePlayer('participant','Player'); persist(); } setupPeer(player.peerId); audio = new PhoneAudio(log); commonChrome(root,'Player Phone'); renderPlayer($('#main')); }
function playerIsJoined(){ return !!(room?.hostPeerId && player?.playerNumber && room.players?.some(p=>p.playerId===player.playerId || p.peerId===player.peerId)); }
function joinRoomHtml(roomCode){ const reconnect = room?.hostPeerId ? `<div class="card"><h2>Reconnect</h2><p>Previously in room <strong>${escapeHtml(room.roomCode)}</strong>. Create a fresh phone pairing code and ask the host for a new answer.</p><button id="forgetRoom">Forget room, start fresh</button></div>` : ''; return `<section class="phone-screen"><details class="card" open><summary>Join room</summary><p class="subtle">Pair this phone with the host. After joining, queue and mic controls appear.</p><label>Room code<input id="roomCode" value="${escapeHtml(roomCode)}" placeholder="Room code"></label><button id="makeOffer" class="primary">Create phone pairing code</button><div id="offerOut"></div><label>Host answer<textarea id="answer" placeholder="Paste host answer/link/chunks"></textarea></label><div class="button-row"><button id="scanAnswerQr">Scan host answer QR</button><button id="importAnswer" class="primary">Finish pairing</button></div></details>${reconnect}</section>`; }
function attachJoinHandlers(){
 ...
 $('#forgetRoom')?.addEventListener('click',()=>{ localStorage.removeItem('carryokie.room'); localStorage.removeItem('carryokie.player'); location.reload(); });
}
...
<details class="card"><summary>Debug room state</summary><pre>${JSON.stringify(room || {status:'not paired'}, null, 2)}</pre></details></section>`;
 document.querySelectorAll('button').forEach(b=>b.addEventListener('click', unlockPhoneAudio));
 $('#requestSong').onclick=()=>{ const item=queueRequest($('#song').value, $('#singers').value.split(',').map(s=>+s.trim()).filter(Boolean), player.playerId, room?.queue?.length || 0); peerNode.broadcast({type:RPC.QUEUE_ADD_REQUEST,item}); log('Queue request sent.'); };
