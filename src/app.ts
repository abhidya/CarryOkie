...
function renderPlayer(main){
 ...
 main.innerHTML = `<section class="phone-screen"><div class="phone-hero card"><p class="eyebrow">CarryOkie phone</p><h2>${currentTitle}</h2><p class="subtle">Room ${escapeHtml(roomCode || 'not paired')} · Player #${escapeHtml(player.playerNumber || '?')}</p><div class="primary-actions"><button id="enableMic" class="primary">Enable mic</button><button id="holdSing" class="hold-button">Hold to sing</button><button id="toggleSing">Toggle live/mute</button><button id="muteMic" class="danger">Mute</button></div><p id="micStatus" class="status-pill">Mic muted until enabled.</p></div>
<details class="card" open><summary>Join / pairing</summary><p class="subtle">Room code opens UI only. WebRTC needs offer/answer exchange.</p><label>Room code<input id="roomCode" value="${escapeHtml(roomCode)}" placeholder="Room code"></label><button id="makeOffer">Create player offer for host</button><div id="offerOut"></div><label>Host answer<textarea id="answer" placeholder="Paste host answer/link/chunks"></textarea></label><button id="scanAnswerQr">Scan host answer QR</button><button id="importAnswer">Import answer</button></details>
<details class="card" open><summary>Queue</summary><label>Song<select id="song">${catalog.map(s=>`<option value="${s.songId}">${escapeHtml(s.title)} — ${escapeHtml(s.artist)}</option>`).join('')}</select></label><label>Singers<input id="singers" value="${player.playerNumber || 2}" placeholder="Singer numbers comma separated"></label><div class="button-row"><button id="requestSong" class="primary">Request song</button><button id="requestSinger">Request singer slot</button></div><div class="queue-list">${queueHtml(room,'phone')}</div></details>
<details class="card" open><summary>Mic setup</summary><p class="warn compact">${singerWarning}</p><label class="check"><input type="checkbox" id="headphones"> Headphones confirmed</label><label class="check"><input type="checkbox" id="pushToSing"> Push-to-sing</label><label>Mic filter<select id="voicePreset"><option value="clean">Clean</option><option value="alto">Alto warm</option><option value="bravo">Bravo bright</option><option value="bass">Bass low</option><option value="radio">Radio</option><option value="autotune">Autotune-style polish</option></select></label><div class="button-row"><button id="startBacking">Start headphone backing monitor</button><button id="pauseBacking">Pause backing monitor</button></div><label>Remote gain <input id="remoteGain" type="range" min="0" max="2" value="1" step=".05"></label><label>Backing monitor gain <input id="backingGain" type="range" min="0" max="1" value="0.35" step=".05"></label><label>Master gain <input id="masterGain" type="range" min="0" max="2" value="1" step=".05"></label><p id="wake" class="subtle"></p></details>
<details class="card"><summary>Lyrics / video sync</summary><video id="phoneVideo" controls playsinline muted></video><div id="lyricsPanel"></div><div class="button-row"><button id="earlier">lyrics -250ms</button><button id="later">lyrics +250ms</button><button id="resetSync">reset sync</button></div></details>
