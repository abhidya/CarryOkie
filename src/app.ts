...
function renderHost(main){ const song = catalog.find(s=>s.songId===room.currentSongId) || catalog[0]; main.innerHTML = `<section class="grid"><div class="card"><h2>Room ${room.roomCode}</h2>  <p>Players ${room.players.length}/5. Active singers max ${MAX_SINGERS}.</p><p><a href="../player/?room=${room.roomCode}">Player join link</a></p><button id="newRoom">New room</button></div><div class="card"><h2>Cast</h2><p id="castState">Cast optional until device/browser supported.</p><input id="castAppId" placeholder="Cast App ID" value="CC1AD845"><button id="castInit">Init Cast</button><button id="castSession">Start Cast</button><button id="castLoad">Load TV media</button><button id="castPlay">Play/Pause</button><button id="castPause">Pause</button><button id="hostPreviewBtn">Preview locally</button><video id="hostPreview" controls playsinline></video><input id="seekSeconds" type="number" min="0" step="1" value="0"><button id="castSeek">Seek seconds</button></div><div class="card"><h2>Manual pairing</h2><textarea id="offer" placeholder="Paste player offer/link/chunks"></textarea><button id="answerOffer">Create answer after complete ICE</button><div id="answerOut"></div></div><div class="card"><h2>Queue</h2>${queueHtml(room)}<button id="acceptAll">Accept all</button><button id="startNext">Start next queued</button></div><div class="card"><h2>Singers</h2>${room.players.map(p=>`<label><input type="checkbox" class="singer" value="${p.playerId}" ${p.isSingerForCurrentSong?'checked':''}> #${p.playerNumber} ${p.displayName}</label><button class="mutePlayer" data-player-id="${p.playerId}">Mute #${p.playerNumber}</button>`).join('')}<button id="setSingers">Set singers</button></div><div class="card"><h2>Playback sync</h2><button id="syncFake">Broadcast TV media sample</button><pre>${JSON.stringify(room.playbackState,null,2)}</pre></div></section>`;
