...
export function makeRoom(hostPlayer: Player): Room {
  return {
    ...
    limits: { maxPlayers: MAX_PLAYERS, maxSingers: MAX_SINGERS },
  };
}
...
export function assignSingers(room: Room, playerIds: string[]): Room {
  if (playerIds.length > MAX_SINGERS)
    throw new Error(`Maximum ${MAX_SINGERS} active singers.`);
...
export function queueRequest(
  ...
  if (singerNumbers.length > MAX_SINGERS)
    throw new Error(`Queue item max ${MAX_SINGERS} singers.`);
  const singers = [
    ...new Set(
      singerNumbers.filter(
        (n) => Number.isInteger(n) && n >= 1 && n <= MAX_PLAYERS,
      ),
    ),
  ];
...
export function addSingerToQueueItem(
  room: Room,
  queueItemId: string,
  singerNumber: number,
): Room {
  ...
  if (
    ...
    singerNumber < 1 ||
    singerNumber > MAX_PLAYERS
  )
    return room;
  if (!item.singerNumbers.includes(singerNumber)) {
    if (item.singerNumbers.length >= MAX_SINGERS)
      throw new Error(`Queue item max ${MAX_SINGERS} singers.`);
