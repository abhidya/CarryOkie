...
function receiverApp(root) {
  ...
  function render() {
    ...
    root.querySelector("#receiverStatus").innerHTML =
      ...
      (resolvedLiveMicStatus
        ? `<p class="status-pill live-status">${escapeHtml$1(resolvedLiveMicStatus)}</p>`
        : "");
  ...
  function liveMicSummaryHtml() {
    const status =
      state.liveMicStatus || (liveMicTrackIds.size ? liveMicStatus() : "");
    return status
      ? `<p class="subtle">${escapeHtml$1(status)}</p>`
  ...
  async function ensureLiveMicOutputGraph() {
    ...
    if (liveMicAudioContext.state === "suspended")
      await liveMicAudioContext.resume().catch((error) => {
