...
var PeerJsRoomTransport = class PeerJsRoomTransport {
	...
	_isHost = false;
	serverConfig;
	constructor(handlers, serverConfig = PeerJsRoomTransport.readServerConfigFromUrl()) {
	...
	*/
	...
	static readServerConfigFromUrl() {
		const config = {};
		try {
	...
	static appendServerConfig(url, serverConfig = PeerJsRoomTransport.readServerConfigFromUrl()) {
		...
		if (typeof serverConfig.secure === "boolean") url.searchParams.set("peerSecure", serverConfig.secure ? "1" : "0");
	...
	*/
	static playerJoinUrl(roomCode, serverConfig = PeerJsRoomTransport.readServerConfigFromUrl()) {
...
function renderHost(main) {
	...
	const hostNextStep = !audioPipeline.receiverReady ? "Open TV Stage, then cast that tab." : !room.queue.length ? "Waiting for singers to scan, join, and queue." : !room.currentQueueItemId ? "Ready. A singer can start the next song from their phone." : "Show is live. Singers can queue and sing.";
