...
test("clock-offset formula matches design.md ping/pong math", () => {
  const { rttMs, hostOffsetMs } = estimateHostOffset({
    clientSentAtMs: 1000,
    hostReceivedAtMs: 1500,
    clientReceivedAtMs: 1100,
  });
...
test("lyric position advances only from sampled Cast media time", () => {
  const state = {
    status: "playing",
    tvMediaTimeMs: 5000,
    tvMediaTimeSampledAtHostMs: 10000,
    playbackRate: 1,
    seekOffsetMs: 250,
    syncDegraded: false,
  };
  ...
  assert.equal(
    deriveTvMediaPositionMs({ ...state, status: "paused" }, 11250, 0)
      .positionMs,
    5250,
  );
  assert.equal(
  ...
  assert.equal(
    deriveTvMediaPositionMs({ ...state, syncDegraded: true }, 11250, 0)
      .syncDegraded,
    true,
  );
});
...
test("manual WebRTC offer-answer flow uses complete ICE payloads", async () => {
  ...
  try {
    ...
    assert.equal(
      player.peers.has(host.localPeerId),
      true,
      "player should re-key the host alias to the actual host peer id",
    );
    assert.equal(
      player.peers.has("host"),
      false,
      "literal host alias must not swallow later renegotiation offers",
    );
  } finally {
...
test("fresh manual offers replace stale peer connections", async () => {
  ...
  try {
    ...
    assert.equal(
      firstEdge.pc.closed,
      true,
      "old pending connection should be closed before re-pairing",
    );
  } finally {
    globalThis.RTCPeerConnection = old;
  }
...
test("local mic stream is added to all current and future peer connections", () => {
  ...
  try {
    ...
    assert.equal(
      later.pc.addedTracks.length,
      1,
      "future peers should receive already-published mic tracks",
    );
...
test("mic tracks renegotiate over the existing host DataChannel", async () => {
  ...
  try {
    ...
    assert.equal(
      edge.negotiating,
      true,
      "renegotiation should stay locked until the answer arrives",
    );
  } finally {
    globalThis.RTCPeerConnection = old;
  }
...
test("renegotiation queues track changes until the prior answer arrives", async () => {
  ...
  try {
    ...
    const stream1 = {
      id: "mic-stream-1",
      getTracks() {
        return [{ kind: "audio", id: "mic-1", enabled: true }];
      },
    };
    const stream2 = {
      id: "mic-stream-2",
      getTracks() {
        return [{ kind: "audio", id: "mic-2", enabled: true }];
      },
    };
    ...
    assert.equal(
      edge.dc.sent.length,
      1,
      "second offer should wait while first offer is unanswered",
    );
    assert.equal(edge.needsNegotiation, true);

    node.handleMessage("host", {
    ...
    assert.equal(
      edge.dc.sent.length,
      2,
      "queued track change should renegotiate after the answer",
    );
...
test("host relays singer mic streams to listener peer connections", async () => {
  ...
  try {
    ...
    await new Promise((resolve) => setTimeout(resolve, 0));
    assert.equal(
      singerEdge.pc.addedTracks?.length || 0,
      0,
      "host must not echo the singer mic back to its source phone",
    );
    assert.equal(
      listenerEdge.pc.addedTracks.length,
      1,
      "listener connection should receive the singer mic track from host",
    );
    assert.equal(listenerEdge.pc.addedTracks[0].stream, stream);
    assert.equal(
      listenerEdge.dc.sent.at(-1).type,
...
test("mic enabled notification broadcasts to every open DataChannel", () => {
  ...
  node.peers.set("host", {
    dc: {
      readyState: "open",
      send: (data) => sent.push(["host", JSON.parse(data)]),
    },
  });
  node.peers.set("listener", {
    dc: {
      readyState: "open",
      send: (data) => sent.push(["listener", JSON.parse(data)]),
    },
  });
  ...
  assert.equal(
    sent.every(
      ([, msg]) => msg.type === RPC.MIC_ENABLED && msg.playerId === "p2",
    ),
    true,
  );
});
...
test("failed ICE emits no-TURN network guidance", () => {
  ...
  const edge = {
    remotePeerId: "b",
    pc: { iceConnectionState: "failed", connectionState: "failed" },
    dc: null,
  };
