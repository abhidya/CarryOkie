...
test("QR SVG renders locally with finder-like dark modules and no remote QR service", async () => {
  const payload = await encodeSignalPayload({
    kind: "offer",
    fromPeerId: "a",
    toPeerId: "b",
...
test("peer-assisted signaling relay forwards non-local relay messages", () => {
  ...
  const sent = [];
  node.peers.set("b", {
    dc: { readyState: "open", send: (data) => sent.push(JSON.parse(data)) },
  });
...
test("relay helper rejects unsupported relay types", () => {
  const node = new PeerNode("host");
  assert.throws(
    () => node.relaySignal("ROOM_HELLO", "a", "b", {}),
    /Unsupported relay type/,
  );
});
...
test("phone mic publishes filtered WebAudio stream and PeerNode routes filtered track", async () => {
  ...
  function node(name) {
    return {
      name,
      connect(target) {
        links.push([name, target.name || "destination"]);
      },
      disconnect() {},
    };
  }
  ...
  globalThis.AudioContext = class {
    ...
    createMediaStreamDestination() {
      return { name: "filteredDestination", stream: filteredStream };
    }
    createBiquadFilter() {
      const n = node("filter");
      n.frequency = { value: 0 };
    ...
    createDynamicsCompressor() {
      ...
      n.attack = { value: 0 };
      n.release = { value: 0 };
      created.compressors.push(n);
      return n;
    }
    createScriptProcessor() {
      return { name: "gate", connect() {}, onaudioprocess: null };
    }
  };
...
test("remote singer audio resumes output graph and keeps source nodes alive", async () => {
  ...
  function node(name) {
    return {
      ...
      gain: { value: 0 },
      connect(target) {
        connected.push([name, target?.name || "dest"]);
      },
    };
  }
  globalThis.AudioContext = class {
  ...
  try {
    ...
    assert.equal(
      resumeCalls,
      1,
      "listener audio should resume after a prior user gesture unlocked it",
    );
    assert.equal(
      audio.remoteSources.length,
      1,
      "remote source node must stay referenced once without duplicate playback",
    );
  ...
  globalThis.AudioContext = class {
    ...
    createGain() {
      return {
        ...
        gain: { value: 0 },
        connect(target) {
          connected.push(["gain", target?.name || "dest"]);
        },
      };
    }
    createMediaStreamSource() {
    ...
    createMediaElementSource(audio) {
      return {
        name: "mediaSource",
        connect(target) {
          connected.push([audio.src, target?.name || "target"]);
        },
      };
    }
  };
...
test("duet monitoring connects and disconnects peer-specific gain", async () => {
  ...
  globalThis.AudioContext = class {
    ...
    createMediaStreamSource() {
      return {
        connect(target) {
          links.push(target?.name || "duetGain");
        },
      };
    }
  };
...
test("payload card buttons navigate QR chunks and copy/share link", async () => {
  ...
  try {
    ...
    renderPayloadCard(
      target,
      { url: "https://x/#signal=abc", token: "abc", chunks: ["one", "two"] },
      "Test payload",
    );
...
test("QR scanner reports unsupported browser and insecure media errors", async () => {
  ...
  try {
    ...
    if (oldDetector) globalThis.BarcodeDetector = oldDetector;
