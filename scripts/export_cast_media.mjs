...
function readKey() {
  ...
  if (key.length !== 32) throw new Error('MEDIA_KEY_B64 must decode to 32 bytes');
  return key;
}
...
function decryptMedia(media, key) {
  ...
  const decipher = crypto.createDecipheriv('aes-256-gcm', key, Buffer.from(media.iv, 'base64'));
  decipher.setAuthTag(tag);
  const plain = Buffer.concat([decipher.update(ciphertext), decipher.final()]);
  const digest = crypto.createHash('sha256').update(plain).digest('hex');
  if (media.plainSha256 && digest !== media.plainSha256) throw new Error(`SHA mismatch for ${media.url}`);
  return plain;
}
