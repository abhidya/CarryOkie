...
const key = Buffer.from(
  keyText.match(/MEDIA_KEY_B64\s*:\s*string\s*=\s*['"]([^'"]+)['"]/)[1],
  "base64",
);
...
for (const song of catalog.songs) {
  ...
  assert.ok(
    song.defaultCastMediaUrl?.startsWith("/public/cast/media/"),
    "default Chromecast clear export required",
  );
  assert.match(path.basename(song.defaultCastMediaUrl), /^[a-f0-9]{32}\.mp4$/);
  assert.equal(song.defaultCastMediaMimeType, "video/mp4");
  assert.ok(
    fs.existsSync(path.join(process.cwd(), song.defaultCastMediaUrl.slice(1))),
    "default cast MP4 file exists",
  );
}
...
const encryptedPath = path.join(
  process.cwd(),
  first.encryptedMedia.url.slice(1),
);
const encrypted = fs.readFileSync(encryptedPath);
assert.notEqual(
  encrypted.subarray(0, 8).toString("utf8"),
  "ftypmp42",
  "encrypted blob should not look like MP4",
);
assert.notEqual(
  encrypted.subarray(4, 8).toString("utf8"),
  "ftyp",
  "encrypted blob should not expose MP4 ftyp header",
);
...
assert.equal(
  plain.subarray(4, 8).toString("utf8"),
  "ftyp",
  "decrypted media should be MP4",
);
...
assert.ok(
  catalog.songs.find((s) => s.encryptedMedia?.url),
  "at least one song has encrypted video media",
);
console.log(
  `PASS protected media obfuscation catalog (${catalog.songs.length} encrypted videos)`,
);
