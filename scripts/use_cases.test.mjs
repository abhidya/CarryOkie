...
test("room cap = 5 players", () => {
  ...
  const room = makeRoom(host);
  for (let i = 0; i < 4; i++)
    addPlayer(room, makePlayer("participant", `P${i}`));
  assert.equal(room.players.length, MAX_PLAYERS);
  assert.throws(
    () => addPlayer(room, makePlayer("participant", "P5")),
    /Room full/,
  );
});
...
test("singer cap = 5 active singers", () => {
  ...
  const p2 = makePlayer("participant", "P2");
  const p3 = makePlayer("participant", "P3");
...
test("queue rejects more than 5 singers", () => {
  assert.throws(
    () => queueRequest("song_001", [1, 2, 3, 4, 5, 6], "p"),
    /max 5 singers/,
  );
});
...
test("queue cap enforced at 20 items", () => {
  ...
  for (let i = 0; i < MAX_QUEUE_ITEMS; i++)
    room.queue.push(queueRequest("song_001", [1], host.playerId, i));
  assert.equal(room.queue.length, MAX_QUEUE_ITEMS);
  assert.throws(
    () => queueRequest("song_001", [1], host.playerId, MAX_QUEUE_ITEMS),
    /Queue full.*20/,
  );
});
...
test("host queue intake normalizes stale phone requests and ignores duplicate submissions", () => {
  ...
  const item = queueRequest(
    "song_001",
    [p1.playerNumber, p1.playerNumber, 99],
    p1.playerId,
    0,
  );
...
test("phones can update queue singer slots before a song starts", () => {
  ...
  assert.deepEqual(room.queue[0].singerNumbers, [
    p1.playerNumber,
    p2.playerNumber,
  ]);
...
test("strict WebRTC config uses public STUN and no TURN", () => {
  assert.deepEqual(rtcConfig.iceServers, [
...
test("manual payload round-trips compressed/base64url and preserves valid SDP", async () => {
  const sdp = [
    ...
    "a=candidate:1 1 udp 1 192.168.1.2 123 typ host generation 0 network-id 1 network-cost 10",
  ...
  assert.ok(
    stripped.length < sdp.length * 0.7,
    `expected stripped SDP to be much smaller (${stripped.length}/${sdp.length})`,
  );
...
test("default QR chunks use larger near-capacity payloads", () => {
  ...
  assert.ok(
    chunks.length <= 5,
    `expected no more than 5 QR chunks, got ${chunks.length}`,
  );
...
test("protected catalog includes songs with encrypted media", () => {
  ...
  assert.match(
    withEncrypted.encryptedMedia.url,
