...
const full = coverage.criteria.filter(
  (c) => c.level === "automated_full",
).length;
const partial = coverage.criteria.filter(
  (c) => c.level === "automated_partial",
).length;
const manual = coverage.criteria.filter(
  (c) => c.level === "manual_hardware_required",
).length;
...
assert.ok(
  full >= 10,
  `expected at least 10 full automated criteria, got ${full}`,
);
assert.ok(
  full + partial >= 19,
  `expected at least 19 criteria with automated evidence, got ${full + partial}`,
);
console.log(
  `PASS design.md coverage map: ${full} full, ${partial} partial, ${manual} hardware/manual`,
);
