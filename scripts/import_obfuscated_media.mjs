...
function readKey() {
  ...
  if (key.length !== 32)
    throw new Error("MEDIA_KEY_B64 must decode to 32 bytes for AES-256-GCM");
  return key;
}
function sha256(buf) {
...
function parseArtistTitle(baseTitle) {
  ...
  if (parts.length >= 2)
    return {
      artist: parts.shift(),
      title: parts.join(" - ") + " (Karaoke Version)",
    };
...
function collectPublicSongs() {
  ...
  return fs
    .readdirSync(PUBLIC_SONGS_DIR, { withFileTypes: true })
    ...
    .map((d) => {
      ...
      return {
        ...
        sourceVideoId: manifest.songId,
        videoFile,
        audioFile: audioFile?.endsWith(".mp3") ? audioFile : null,
...
if (!sources.length)
  throw new Error(
    "No MP4 files found in downloads/mp4 or public/songs manifests",
  );
...
for (const source of sources) {
  try {
    const song = {
      ...
      licenseInfo:
        "user-provided/locally imported media; verify rights before deployment",
      encryptedMedia: writeEncrypted(source.videoFile, key, "video/mp4"),
    };
    if (source.audioFile)
      song.encryptedAudio = writeEncrypted(source.audioFile, key, "audio/mpeg");
    songs.push(song);
  } catch (err) {
    const reason = err?.message || `${err}`;
    console.error(
      `SKIP ${source.songId}: ${reason} (file: ${source.videoFile})`,
    );
  ...
  JSON.stringify(
    {
      ...
      sources: ["public/songs", "downloads/mp4"],
      songs,
    },
    null,
    2,
...
console.log(
  `Imported ${songs.length} encrypted lyric videos into ${MEDIA_DIR}`,
);
if (failures.length) {
  console.error(
    `${failures.length} skipped (see above): ${failures.map((f) => f.songId).join(", ")}`,
  );
  process.exitCode = 1;
}
