...
const badHtmlRefs = htmlFiles.flatMap(([file, html]) => {
  const bad = [];
  if (/\b(?:src|href)=['"]\/(?:assets|src)\//.test(html))
    bad.push("root-relative asset/source URL");
  if (
    /\b(?:src|href)=['"][^'"]*(?:main\.ts|styles\.css)/.test(html) &&
    !/assets\//.test(html)
  )
    bad.push("unbundled source URL");
  return bad.map((reason) => `${file}: ${reason}`);
});
if (badHtmlRefs.length) {
  console.error(
    "Built HTML contains GitHub Pages-hostile references:\n" +
      badHtmlRefs.join("\n"),
  );
  process.exit(1);
}
...
const badPublicRefs = [
  ...
  "/public/cast",
  "../public/songs",
].filter((ref) => builtJs.includes(ref));
if (badPublicRefs.length) {
  console.error(
    "Built JS contains public-directory references that do not exist in dist: " +
      badPublicRefs.join(", "),
  );
  process.exit(1);
}
...
if (!fs.existsSync("public/protected/catalog.json")) {
  console.error(
    "Missing public/protected/catalog.json - run npm run importMedia first",
  );
  process.exit(1);
}
...
if (missingCast.length) {
  console.error(
    "Missing Default Cast media exports for: " +
      missingCast.map((s) => s.songId).join(", "),
  );
...
console.log(
  "✓ Static build OK: GitHub Pages can serve dist at /CarryOkie/ plus /host /player /receiver /debug",
);
