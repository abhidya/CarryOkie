...
checks.push(['host exposes progressive setup and tab-cast receiver link', app.includes('setupComplete') && app.includes('Open TV receiver tab') && app.includes('receiverUrl()') && app.includes('setupReceiverBridge') && app.includes('RECEIVER_PLAYBACK_SYNC') && app.includes('publishReceiverCommand') && app.includes('receiverPendingRenegotiate') && app.includes('song:currentSong()')]);
checks.push(['player only shows join controls before pairing', app.includes('playerIsJoined()') && app.includes('joinRoomHtml') && app.includes('After joining, queue and mic controls appear')]);
...
checks.push(['cast load autoplays TV media', fs.readFileSync('src/cast.ts','utf8').includes('request.autoplay = true') && fs.readFileSync('src/cast.ts','utf8').includes('await this.play()') && fs.readFileSync('src/cast.ts','utf8').includes('media.play().then') && fs.readFileSync('src/cast.ts','utf8').includes('Tap receiver once to start backing track/audio')]);
...
checks.push(['player has reconnect UI', app.includes('Reconnect') && app.includes('Forget room, start fresh') && app.includes('forgetRoom')]);
...
checks.push(['receiver renders join QR lyrics singers and tab-cast live mics', cast.includes("import { qrSvg }") && cast.includes('id="joinQr"') && cast.includes('id="liveMics"') && cast.includes('id="receiverStatus"') && cast.includes('receiverId') && cast.includes('RECEIVER_OFFER') && cast.includes('RECEIVER_PLAYBACK_SYNC') && cast.includes('CAST_SET_SINGERS') && cast.includes('CAST_SYNC_PLAYBACK_STATE') && cast.includes('loadLyrics') && cast.includes('timeupdate')]);
