...
function setupTestEnv({ goodCount = 3, badCount = 1, withAudio = false } = {}) {
  ...
  if (withAudio) fs.writeFileSync(path.join(realSongDir, 'backing.mp3'), makeMp3());
  fs.writeFileSync(path.join(realSongDir, 'manifest.json'), JSON.stringify({
    ...manifest,
    castMediaUrl: `/songs/song_demo/video.mp4`,
    phoneBackingAudioUrl: withAudio ? `/songs/song_demo/backing.mp3` : undefined,
  }, null, 2));
...
function cleanupTestEnv() {
  fs.rmSync(TEST_DIR, { recursive: true, force: true });
  fs.rmSync(path.join(ROOT, 'songs', 'song_demo'), { recursive: true, force: true });
}
...
test('Design §15.1: importMedia only processes allowed sources (downloads/mp4 + public/songs)', () => {
  assert.ok(fs.existsSync('scripts/import_obfuscated_media.mjs'), 'import script exists');
  ...
  assert.match(readme, /Spotify.*excluded|unlicensed media excluded/i, 'README excludes Spotify/unlicensed media');
});
...
test('Design §15.2: importMedia produces correct repo structure (protected/catalog.json + media/*.bin)', () => {
  assert.ok(fs.existsSync('public/protected/catalog.json'), 'catalog.json must exist after importMedia');
  ...
  for (const song of catalog.songs) {
    assert.ok(song.encryptedMedia?.url, `${song.songId}: has encryptedMedia.url`);
    const binPath = path.join(ROOT, song.encryptedMedia.url.replace(/^\//, ''));
    assert.ok(fs.existsSync(binPath), `${song.songId}: encrypted .bin file exists at ${binPath}`);
    assert.match(path.basename(binPath), /^[a-f0-9]+\.bin$/, `${song.songId}: .bin has random hex name`);
  }
});
...
test('Design §15.3: Protected catalog song entries match SongManifest shape from design §10.3/§15.3', () => {
  ...
  for (const song of catalog.songs) {
    ...
    assert.equal(song.encryptedMedia?.algorithm, 'AES-256-GCM', `${song.songId}: algorithm is AES-256-GCM`);
    assert.ok(song.encryptedMedia?.iv, `${song.songId}: has IV`);
    assert.equal(song.encryptedMedia?.tagBytesAppended, 16, `${song.songId}: tagBytesAppended is 16`);
    assert.ok(song.encryptedMedia?.mimeType, `${song.songId}: has mimeType`);
    assert.ok(song.encryptedMedia?.plainSha256, `${song.songId}: has plainSha256`);
    assert.ok(typeof song.encryptedMedia?.plainByteLength === 'number', `${song.songId}: has plainByteLength`);
  }
});
...
test('Encryption: AES-256-GCM encrypt → decrypt round-trip produces identical plaintext', () => {
  ...
  const plain = decryptBin(encrypted, first.encryptedMedia.iv, key, first.encryptedMedia.tagBytesAppended);
  const digest = crypto.createHash('sha256').update(plain).digest('hex');
  assert.equal(digest, first.encryptedMedia.plainSha256, 'decrypted SHA-256 matches catalog plainSha256');
});
...
test('Obfuscation: encrypted .bin files must not expose MP4 ftyp header', () => {
  ...
  assert.notEqual(encrypted.subarray(4, 8).toString('ascii'), 'ftyp', 'encrypted blob must not expose ftyp header');
  assert.notEqual(encrypted.subarray(0, 4).toString('ascii'), 'ftyp', 'encrypted blob must not start with ftyp');
});
...
test('Obfuscation: .bin filenames are random hex (not guessable from songId/title)', () => {
  ...
  for (const song of catalog.songs) {
    ...
    assert.match(base, /^[a-f0-9]{32}\.bin$/, `${song.songId}: .bin name is 32-hex-char random`);
    assert.ok(!base.includes(song.songId), `${song.songId}: .bin name does not contain songId`);
    assert.ok(!base.includes(song.title?.replace(/\s/g, '')), `${song.songId}: .bin name does not contain title`);
  }
});
...
test('Design §4.2: exportCastMedia creates clear MP4 for Chromecast Default Media Receiver', () => {
  const catalog = JSON.parse(fs.readFileSync('public/protected/catalog.json', 'utf8'));
  for (const song of catalog.songs) {
    assert.ok(song.defaultCastMediaUrl, `${song.songId}: has defaultCastMediaUrl`);
    assert.match(song.defaultCastMediaUrl, /\/public\/cast\/media\//, `${song.songId}: cast URL points to cast/media/`);
    assert.match(path.basename(song.defaultCastMediaUrl), /^[a-f0-9]+\.mp4$/, `${song.songId}: cast file has random hex .mp4 name`);

    const castPath = path.join(ROOT, song.defaultCastMediaUrl.replace(/^\//, ''));
    ...
    assert.equal(buf.subarray(4, 8).toString('ascii'), 'ftyp', `${song.songId}: cast .mp4 has valid MP4 header`);
  }
});
...
test('Cast export: stable filename derived from songId + plainSha256', () => {
  ...
  assert.equal(path.basename(first.defaultCastMediaUrl), expected, 'cast filename is deterministic from songId+sha256');
});
...
test('Client decrypt: src/protectedMedia.ts uses Web Crypto AES-GCM matching server encryption', () => {
  ...
  assert.match(code, /loadProtectedCatalog/, 'has loadProtectedCatalog function');
...
test('Auto-skip: individual song failures are logged and skipped, remaining songs succeed', () => {
  ...
  assert.match(code, /catch\s*\(err\)/, 'script has try/catch for individual songs');
  assert.match(code, /SKIP/, 'script logs SKIP on failure');
  assert.match(code, /failures/, 'script tracks failures array');
  assert.match(code, /process\.exitCode\s*=\s*1/, 'script sets exitCode 1 on failures');

  // Verify sources.map is NOT used (replaced with for...of + try/catch)
  assert.doesNotMatch(code, /sources\.map\s*\(/, 'script does not use sources.map (would not allow try/catch per item)');
});
...
test('Catalog integrity: every encryptedMedia .bin URL in catalog maps to an existing file', () => {
  ...
  for (const song of catalog.songs) {
    ...
    if (song.encryptedAudio?.url) {
      const binPath = path.join(ROOT, song.encryptedAudio.url.replace(/^\//, ''));
      assert.ok(fs.existsSync(binPath), `${song.songId}: audio ${binPath} exists`);
    }
  }
});
...
test('Catalog integrity: no orphan .bin files in protected/media/ (all referenced by catalog)', () => {
  ...
  for (const song of catalog.songs) {
    if (song.encryptedMedia?.url) referencedBins.add(path.basename(song.encryptedMedia.url));
    if (song.encryptedAudio?.url) referencedBins.add(path.basename(song.encryptedAudio.url));
  }

  const diskBins = fs.readdirSync(mediaDir).filter(f => f.endsWith('.bin'));
  for (const f of diskBins) {
    assert.ok(referencedBins.has(f), `orphan .bin on disk not in catalog: ${f}`);
  }
  assert.equal(diskBins.length, referencedBins.size, 'disk .bin count matches catalog references');
});
...
test('Key file: src/mediaKey.ts exports valid 32-byte base64 key for AES-256-GCM', () => {
  ...
  assert.match(text, /MEDIA_KEY_B64\s*:\s*string\s*=\s*['"][A-Za-z0-9+/=]+['"]/, 'exports MEDIA_KEY_B64 as string');
  const key = readKey();
...
test('Source tracking: catalog songs record source field (public/songs or downloads/mp4)', () => {
  ...
  for (const song of catalog.songs) {
    ...
    assert.ok(['public/songs', 'downloads/mp4'].includes(song.source), `${song.songId}: source is valid`);
  }
  assert.ok(catalog.sources, 'catalog has top-level sources array');
  assert.ok(catalog.sources.includes('public/songs'), 'sources includes public/songs');
  assert.ok(catalog.sources.includes('downloads/mp4'), 'sources includes downloads/mp4');
});
...
test('Design §15.1/§15.3: every song has licenseInfo (rights verification before deployment)', () => {
  ...
  for (const song of catalog.songs) {
    assert.ok(song.licenseInfo, `${song.songId}: has licenseInfo`);
    assert.match(song.licenseInfo, /user-provided|licensed|rights/i, `${song.songId}: licenseInfo mentions rights/licensing`);
  }
});
...
test('npm script: importMedia runs import_obfuscated_media.mjs then export_cast_media.mjs', () => {
  ...
  assert.match(pkg.scripts.importMedia, /import_obfuscated_media/, 'runs import script');
  assert.match(pkg.scripts.importMedia, /export_cast_media/, 'runs export script');
