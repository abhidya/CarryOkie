...
- Shape/radius/elevation: moderate rounded cards; shadows only for hierarchy, not decoration everywhere.
...
- Screen-reader semantics: QR payloads include readable text fallback; status logs remain text.
...
- Success: show pairing ready, queue sent/accepted/active, mic live, receiver bridge status.
- Disabled: disabled actions must reflect missing prerequisite.
...
- Compatibility constraints: GitHub Pages/static; localhost/HTTPS for mic/crypto; Default Media Receiver needs clear cast exports.
...
# Legacy implementation handoff preserved from previous DESIGN.md
